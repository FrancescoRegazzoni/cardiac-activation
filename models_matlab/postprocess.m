...
function postprocess(output)
    ...
    subplot(2,2,1);
    plot(output.times, output.Ca, 'linewidth', 2)
    ylim([0 1.2])
    ylabel('[Ca^{2+}]_i [\muM]')

    subplot(2,2,2);
    plot(output.times, output.SL, 'linewidth', 2)
    ylim([2 2.3])
    ylabel('SL [\mum]')

    subplot(2,2,3);
    plot(output.times, output.P, 'linewidth', 2)
    ylim([0 1])
    ylabel('permissivity [-]')

    subplot(2,2,4);
    plot(output.times, output.Ta, 'linewidth', 2)
    ylim([0 80])
